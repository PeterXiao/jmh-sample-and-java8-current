package versions.java8.benchmark;
...
public class ClashOfLambdas {
    ...
    static {
        ...
        refs = IntStream.range(0, N).mapToObj(n -> new ClashOfLambdas.Ref(n))
            .toArray(size -> new ClashOfLambdas.Ref[size]);
    }
    ...
    @Benchmark
    public long cartSeq() {
        long cart = LongStream.of(valuesHi).flatMap(d -> LongStream.of(valuesLo).map(dP -> dP * d)).sum();
        return cart;
    }

    @Benchmark
    public long cartPar() {
        long cart = LongStream.of(valuesHi).parallel().flatMap(d -> LongStream.of(valuesLo).map(dP -> dP * d)).sum();
    ...
    public long sumOfSquaresEvenSeq() {
        long sum = LongStream.of(v).filter(x -> x % 2 == 0).map(x -> x * x).sum();
    ...
    public long sumOfSquaresEvenPar() {
        long sum = LongStream.of(v).parallel().filter(x -> x % 2 == 0).map(x -> x * x).sum();
    ...
    @Benchmark
    public long refSeq() {
        long length = Stream.of(refs).filter(box -> box.num % 5 == 0).filter(box -> box.num % 7 == 0).count();
    ...
    public long refPar() {
        long length =
            Stream.of(refs).parallel().filter(box -> box.num % 5 == 0).filter(box -> box.num % 7 == 0).count();
