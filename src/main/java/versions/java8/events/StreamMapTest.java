...
import versions.java8.modle.Warrior;
...
public class StreamMapTest {
    ...
    public static void main(String[] args) {
        ...
        Stream<Warrior> warriors = persons.stream().filter(p -> p.getAge() > 20 && p.getAge() < 40)
        ...
        warriors = persons.stream().filter(p -> p.getAge() > 20 && p.getAge() < 40)
            // a simple lambda that call the Warrior's constructor
            .map(person -> new Warrior(person)); // here the lambda expression (anonymous method) is passed as an
                                                 // argument
        warriors.forEach((w) -> System.out.println(w));

        System.out.println("\nMap operation using lambda syntax using the method reference: ");
        warriors = persons.stream().filter(p -> p.getAge() > 20 && p.getAge() < 40)
            // the compiler will figure out which is the type of arg to construct and will use each element from the
            // list
            .map(Warrior::new); // <Class>:new is a method reference to constructor (used instead of a simple lambda to
